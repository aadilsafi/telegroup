...
if (count($sql_query) > 0) {
    ...
    $where=['msgs.cat' => 'user','msgs.type[!](not_type)' => 'system','msgs.type[!](not_system)' => 'stickers', 'LIMIT' => 250];
    ...
    foreach ($messages as $message) {
        ...
        if (isset($gid[1])) {
            ...
            $private_conversation_id = DB::connect()->select("gr_private_conversations", ['private_conversation_id'], $where);
        ...
        if (empty($private_conversation_id)) {
            if (!empty($recipient_user_id) && !empty($initiator_user_id)) {
                $insert_data=array();
        ...
        if (!empty($private_conversation_id) && !empty($message['gid']) && !empty($message['uid'])) {
            ...
            if (!empty($msg) || !empty($attachment_type) && !empty($attachments)) {
                ...
                $insert_data=array();
                $insert_data['private_chat_message_id'] = $message['id'];
                $insert_data['private_conversation_id'] = $private_conversation_id;
                ...
                DB::connect()->insert("gr_private_chat_messages", $insert_data);
            }
    ...
    $where=['msgs.cat' => ['group','user'],'msgs.type[!]' => 'system','msgs.type[!]' => 'stickers', 'LIMIT' => 50];
    $total_messages = DB::connect()->count("old_version_gr_msgs(msgs)", $where);

    if (empty($total_messages)) {
        DB::connect()->query("DROP TABLE old_version_gr_msgs");
    }
}
...
if (isset($messages) && count($messages) > 150) {
    ...
    $system_message = 'Importing Custom Fields';
    $redirect = 'update_database';
    $sub_process = 'custom_fields';
}
