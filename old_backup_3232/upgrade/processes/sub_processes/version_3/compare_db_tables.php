...
$search_query = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '".$GLOBALS["database_info"]["database"]."' ";
$search_query .= "AND TABLE_NAME LIKE 'new_grupo_version_%'  LIMIT 0,5;";
$db_tables=  DB::connect()->query($search_query)->fetchAll();
        ...
        if (count($check_db) === 0) {
            ...
            foreach ($table_constraints as $table_constraint) {
                if (!empty($table_constraint['CONSTRAINT_NAME']) && !empty($table_constraint['REFERENCED_COLUMN_NAME'])) {
                    ...
                    $rename_table .= 'FOREIGN KEY (`user_id`) REFERENCES `'.$reference_table_name.'`(`'.$table_constraint['REFERENCED_COLUMN_NAME'].'`) ';
            ...
            foreach ($new_db_table as $column) {
                ...
                if (!isset($old_db_table_columns[$check_column])) {
                    ...
                    if (!empty($column['COLUMN_DEFAULT']) || is_numeric($column['COLUMN_DEFAULT'])) {
                        if (strpos($column['COLUMN_DEFAULT'], "'") !== false || is_numeric($column['COLUMN_DEFAULT']) || $column['COLUMN_DEFAULT'] === 'NULL') {
                            $default_value = " DEFAULT ".$column['COLUMN_DEFAULT'];
                        } else {
                            $default_value = " DEFAULT '".$column['COLUMN_DEFAULT']."'";
                        }
                    }
